...
from app.rate_limiter import (check_single_execution, check_suite_execution, check_nl_execution,release_execution, RateLimitExceeded)
...
# Per-base_url threading Events for suite execution cancellation
_CANCEL_EVENTS: Dict[str, threading.Event] = {}

# Per-url threading Events for NL execution cancellation
_NL_CANCEL_EVENTS: Dict[str, threading.Event] = {}
...
async def abort_suite_execution(payload: StopSuiteRequest):
    """Registers cancellation for both suite Playwright loop and NL agent loop."""
    ACTIVE_CANCELLATIONS.add(payload.base_url)
    if payload.base_url in _CANCEL_EVENTS:
        _CANCEL_EVENTS[payload.base_url].set()
    # Also signal NL executor if it's running for this URL
    if payload.base_url in _NL_CANCEL_EVENTS:
        _NL_CANCEL_EVENTS[payload.base_url].set()
...
@router.post("/nl")
async def execute_nl_test(payload: ExecuteNLRequest, current_user=Depends(get_current_user)):
    """
    ...
    # Rate limit check
    try:
        check_nl_execution(current_user.id)
    ...
    executor = NLExecutor()
    # Register a cancel event for this URL so /execute/stop can signal it
    nl_cancel_event = threading.Event()
    _NL_CANCEL_EVENTS[payload.url] = nl_cancel_event
    saved_run = None
    created_run_log = None
    try:
        result = await executor.execute_autonomous_goal(
            ...
            cancel_event=nl_cancel_event
        )

        # If aborted — clean up any media files and return aborted signal
        if result.get("aborted"):
            for path in result.get("screenshot_paths", []):
                _delete_media_file(path)
            _delete_media_file(result.get("video_path"))
            return {"aborted": True, "mode": "autonomous_agent_goal_run", "run_id": None}

        # 💾 PERMANENT DATABASE RECORD INTERCEPTION
        saved_run = await db.testrun.create(data={
            "filename": f"NL: {full_goal_statement}",
            "total": 1,
        ...
        _NL_CANCEL_EVENTS.pop(payload.url, None)
        release_execution(current_user.id)
...
@router.post("/single")
async def execute_single_direct(payload: DirectSingleRequest, current_user=Depends(get_current_user)):
    """SINGLE TEST - headful first (user watches live), then headless for screenshots."""
    try:
        check_single_execution(current_user.id)
    except RateLimitExceeded as e:
        raise HTTPException(status_code=429, detail=str(e), headers={"Retry-After": str(e.retry_after)})
    ...
    try:
        ...
        # Always release the concurrency slot, success or failure. Previously
        # this was only called on the success path (after the return dict was
        # built), so any exception from either run_test_case call — the exact
        # thing that was happening every time under Docker — left the slot
        # permanently leaked. Fixed independently of the headful/headless
        # question: this matters regardless of which environment this runs in.
        release_execution(current_user.id)


@router.post("/suite")
async def execute_suite_direct(payload: DirectSuiteRequest, current_user=Depends(get_current_user)):
    """SUITE RUN - headless + screenshots with real-time memory and storage reclamation."""
    try:
        check_suite_execution(current_user.id)
    except RateLimitExceeded as e:
        raise HTTPException(status_code=429, detail=str(e), headers={"Retry-After": str(e.retry_after)})
    ...
    try:
        ...
        # Always release the concurrency slot — this used to only
        # happen on the final success return, so the early "aborted"
        # return and any mid-loop exception (e.g. run_test_case failing)
        # both leaked the slot permanently, matching the same bug fixed
        # in execute_single_direct above.
        release_execution(current_user.id)